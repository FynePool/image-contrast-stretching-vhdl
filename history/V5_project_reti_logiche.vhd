...
architecture Behavioral of project_reti_logiche is
    ...
    type STATE_type is (RESET, IDLE , SEND_ADDR, READ,SETUP, ELAB ,WRITE2, WRITE ,ENDWR);
    signal MEM                              : MEM_type ;
    ...
    --used for algorithm:
    signal delta_value                      : std_logic_vector(7 downto 0);   -- difference between max pixel value and min pixel value
              ...
              if i_rst='1' then --Definition of reset
                  ...
                  case STATE is
                    ...
                    when SEND_ADDR =>
                        ...
                        STATE <= READ;
                        if to_integer(unsigned(addr_int)) > img_dim+1 and img_dim/=0 then                                                  --Recognize end of image bytes
                    ...
                    when READ =>
                        ...
                        if i=1 then
                            ...
                            if  to_integer(unsigned(MEM(0))) = 0 then
                                STATE <= ENDWR;
                            else    
                                i <= i+1;
                                STATE <= SEND_ADDR;
                            ...
                            addr_int <= std_logic_vector(to_unsigned((to_integer(unsigned(addr_int)+1)),addr_int'length ));
                            if  to_integer(unsigned(MEM(1))) = 0 then
                                STATE <= ENDWR;
                            else
                                i <= i+1;
                                STATE <= SEND_ADDR;
